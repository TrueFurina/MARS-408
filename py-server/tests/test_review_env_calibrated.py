...
def test_gradient_exists_between_actions():
    """可学梯度：trust_consensus > balanced > trust_honest（均值口径）。"""
    sums = {a: 0.0 for a in range(4)}
    for t in range(_N_TRIALS):
        for a in range(4):
            e = _fresh(30000 + t)
            _f, r, _d = e.step(a)
            sums[a] += r
    m = {a: sums[a] / _N_TRIALS for a in range(4)}
    assert m[2] > m[3] > m[0], f"动作梯度不成立: {m}"


def test_oracle_action_varies_and_beats_best_constant():
    """上下文确实重要：oracle 平均增益显著高于最优恒定动作（否则无可学空间）。"""
    oracle_sum, const_sum = 0.0, {a: 0.0 for a in range(4)}
    seen = set()
    for t in range(_N_TRIALS):
        e = _fresh(40000 + t)
        a_star = oracle_action(e)
        seen.add(a_star)
        values = {a: weighted_consistency_score(e._evidence, e._consensus,
                                                review_weight_schema(_weights_of(a)))[0]
                  for a in range(4)}
        base = values[3]
        oracle_sum += values[a_star] - base
        for a in range(4):
            const_sum[a] += values[a] - base
    assert len(seen) >= 2, "oracle 动作应当随上下文变化（否则无可学信号）"
    best_const = max(const_sum.values()) / _N_TRIALS
    best_oracle = oracle_sum / _N_TRIALS
...
def test_discipline_gate_matches_production_semantics():
    """与 review_policy.select_action 内 _block_skip 语义一致。"""
