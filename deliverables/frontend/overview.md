...
2. **`getFallback*` 本来就返回空数组。** 真实缺陷是「会话拉取」与「推荐任务」两个接口里
   `if (data.length >= 3)` 的判断 —— 真实数据不足 3 条就被整批丢弃。
...
| 指标 | 旧（demo 合成） | 真产物 |
|---|---|---|
| Top-5 召回率 | 92.4% vs 64.3%（+28pp） | 82.14% vs 75.00%（**+7.14pp**） |
| 平均延迟 | 143ms vs 59ms（2.4×） | 574.36ms vs 2.70ms（**≈213×**） |
| Token 变化 | 隐含"省 34% 噪声" | **−11.99%（未降反增）** |
| 矛盾检出数 | 40 vs 0 | **字段不存在**，已移除 |

> ⚠️ 口径说明：上表"真产物"列于 2026-09-22 同步至 `benchmark_2026-09-18.json`（09-18 完整真重跑；实验2 与 9/17 复现逐位一致，实验1 因 FrugalRAG 代码与 torch/numpy 版本演化而诚实漂移）。

结论：真实实验仍支持"FrugalRAG 检索质量更优、NeuralMixer 共识准确率更高（88.89% vs 76.67%）"，
但幅度远小于旧文案，且伴随显著延迟代价（≈213×）。页面已载明。
