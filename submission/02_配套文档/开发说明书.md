...
> 闽江大学网络空间安全学院
> 版本：v2.0 | 2026-07-20（2026-09-19 对齐火山杯参赛口径与代码真值）
>
> 注：MARS-408 为本系统的技术底座代号（多智能体个性化学习系统）。本项目源于国家级大学生创新创业训练计划，参加 2026 福建高校「火山杯」Agent 创新大赛。
...
本项目参加 2026 福建高校「火山杯」Agent 创新大赛。系统围绕"基于大模型的个性化资源生成与学习多智能体系统"方向建设，核心能力对照如下：

| 能力要求 | 本项目实现 | 对应模块 |
|---------|-----------|---------|
| 对话式画像构建（≥6维度） | 8 维动态画像，随学随新 | `agents/diagnostician.py`, `api/profile.py` |
| 多智能体资源生成（≥5种） | 7 种资源类型并行生成 | `agents/generator_cluster.py` + 7 个子 Agent |
| 个性化学习路径与推送 | 86 节点知识图谱 + 82 条先修依赖边 + 8 维画像驱动 | `agents/path_planner.py`, `api/learning_path.py` |
| 智能辅导 | 文字+SVG+短视频脚本多模态答疑 | `agents/tutor.py`, `agents/tutor_enhanced.py` |
| 学习效果评估闭环 | 多维评估 → 回写画像 → 动态调整路径 | `api/assessment.py`, `agents/feedback_agent.py` |
| UI 规范 | 毛玻璃深色设计系统 + SSE 流式输出 | 设计系统 `DESIGN.md` |
| 防幻觉 | RAG + Critic 审阅 + 证据校验 + 质量闸门 | 全链路 4 层防护 |
| 性能 | SSE 流式 + 实时进度追踪 | `api/chat.py`, `api/langgraph.py` |
| 合规标注 | 开源协议 + 讯飞工具声明 | `02_配套文档/OPENSOURCE_LICENSES.md` |
...
| 408 领域知识图谱 + 教学 DAG | 86 节点知识图谱 + 82 条先修关系边 + 648 行知识点依赖规则引擎 | 竞品缺乏 408 四科结构化知识图谱 |
...
本项目深度集成科大讯飞开放平台 **10 项 AI 能力**（TTI 图片生成 / 图片理解 / 聚合搜索 / 智能 PPT / 数字人视频 / 文本纠错 / 公文校对 / 文本合规 / 角色模拟 / 智能简历）+ 星火大模型（generalv3.5），通过正规渠道接入，遵守讯飞开放平台服务协议。TTS 语音合成采用本地开源 MeloTTS 方案。
...
| 序号 | Agent | 文件 | 职责 |
|:----:|------|------|------|
| 1 | Triage | `agents/triage.py` | 请求分诊，零 LLM 成本分类与 low 短路快路径 |
| 2 | Coordinator | `agents/coordinator.py` | 全局协调，解析用户意图，分配任务 |
| 3 | Diagnostician | `agents/diagnostician.py` | 学情诊断，分析学生画像，识别薄弱点 |
| 4 | Planner | `agents/planner.py` | 任务规划，拆解学习目标为子任务 |
| 5 | Retriever | `agents/retriever.py` | FrugalRAG 知识检索，获取相关教学资料 |
| 6 | Generator Cluster | `agents/generator_cluster.py` | 7 个子 Agent 并行生成教学资源 |
| 7 | Assessor | `agents/assessor.py` | 评估反馈，对生成结果进行质量评分 |
| 8 | Critic | `agents/critic.py` | 质量审阅，事实核查 + 敏感词过滤 |
| 9 | Evidence Check | `agents/evidence_check.py` | 证据校验，知识一致性 + 矛盾检测 |
| 10 | Quality Gate | `agents/quality_gate.py` | 产物验收闸门，硬性阻断不合格产物 |
| 11 | PathPlanner | `agents/path_planner.py` | 路径规划，基于画像 + 知识图谱生成学习路径 |
...
| 文件 | 作用 | 行数 |
|------|------|:----:|
| main.py | FastAPI 入口，43 路由模块挂载 | ~500 |
| config.py | 统一配置管理（三通道LLM+向量库+PG+Redis） | ~250 |
| agents/graph.py | LangGraph 11 节点 StateGraph + 条件边 | ~180 |
...
| engines/teaching_rules.py | 408 教学业务规则引擎（648 行） | ~648 |
...
| views/KnowledgeGraphView.vue | 86 节点知识图谱可视化 |
...
![图3 知识图谱（86 节点可视化）](images/image18.jpeg)
...
系统共 **43 个 API 路由模块**，双挂载在 `/api/v1` 和 `/api` 下，包含 451 个端点，97.8% 认证覆盖率。
...
4. **教学规则引擎**：648 行 408 知识点依赖图，校验 Agent 分配是否符合教学逻辑
5. **EWMA 动态权重**：基于历史表现的指数移动平均动态调整各 Agent 权重

### 9.3 408 教学业务规则引擎

`engines/teaching_rules.py`（648 行），覆盖 408 四科：
...
| v2.0 | 2026-07-20 | 全面修订：新增需求分析章节、智能体设计方法论、开源声明前置、新模块覆盖（quality_gate/achievement/ForceGraph/PortalCards）、数据全量对齐代码实际值（讯飞X2 第一优先级） |
| v2.1 | 2026-09-19 | 火山杯参赛口径统一：赛事头改为「火山杯」参赛作品、补 MARS-408 技术底座代号澄清；数字对齐代码真值——知识图谱订正为 86 节点/82 边（原 613/609 为膨胀误写）、LangGraph 节点数订正为 11（补 Triage 节点）、路由模块订正为 43（端点 451）、规则引擎行数订正为 648（原 610）；测试章节对齐全量回归口径 |
